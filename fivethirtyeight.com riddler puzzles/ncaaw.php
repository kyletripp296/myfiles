...
/*
//create table - done
$sql = "drop table ncaaw_2019";
$mysqli->query($sql);
$sql = "create table ncaaw_2019 (winner varchar(256) NOT NULL, loser varchar(256) NOT NULL)";
$mysqli->query($sql);



//insert rows - done
$myfile = fopen("ncaaw.txt", "r") or die("Unable to open file!");
while($line = fgets($myfile)){
	...
	$sql = "insert into ncaaw_2019 values ('$winner_sql','$loser_sql')";
	$mysqli->query($sql);
}
*/

$allteams = array();
$sql = "select distinct loser from ncaaw_2019 order by loser asc";
...
$sql = "select count(1) as count from ncaaw_2019";
$result = $mysqli->query($sql);
if($result->num_rows){
	while($row = $result->fetch_assoc()){
...
$allteams['Baylor'] = false;
$tiers = array(array('Baylor'));

for($i=0;$i<=count($tiers);$i++){
	if(count($tiers[$i])){
		foreach($tiers[$i] as $team){
			$team_sql = $mysqli->real_escape_string($team);
			$sql = "select distinct winner from ncaaw_2019 where loser='$team_sql'";
			$result = $mysqli->query($sql);
			if($result->num_rows){
				while($row = $result->fetch_assoc()){
